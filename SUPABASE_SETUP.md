...
> ## ⚠ 이 문서의 SQL은 더 이상 쓰지 마세요 (2026-09-05)
>
> 아래 SQL에는 **보안 결함과 낡은 쇼 정보**가 있습니다. 그대로 실행하면
> 예약번호를 무차별 대입해 관람객 명단을 통째로 가져갈 수 있고,
> 쇼가 12개(옛 일정)로 들어가 S13 예약이 실패합니다.
>
> **실제로 적용된 스키마는 [`supabase/schema.sql`](supabase/schema.sql) 입니다.**
> 현장 스탠드석 집계는 [`supabase/walkins.sql`](supabase/walkins.sql).
> 이 문서는 설계 배경을 읽는 용도로만 남겨 둡니다.
